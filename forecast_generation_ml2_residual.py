...
parser.add_argument("--noise-std", type=float, default=0.0, help="Noise scale (see --noise-mode).")
...
parser.add_argument(
    "--noise-mode",
    choices=["none", "gaussian", "residual"],
    default="residual",
    help="Noise model: none, multiplicative Gaussian, or additive residual sampling from training errors.",
)
parser.add_argument(
    "--val-start-year",
    type=int,
...
for subsys, sub_df in monthly.groupby("subsystem"):
    ...
    for model_type in MODEL_TYPES:
        ...
        if preds and model_type == args.model:
            ...
            # store residuals for noise sampling (only for the chosen output model)
            train_pred = model_full.predict(X_all)
            res = y_all - train_pred
            res = res[np.isfinite(res)]
            residuals_by_subsys[subsys] = res
...
if args.noise_std and args.noise_std > 0:
    ...
    elif args.noise_mode == "residual":
        def add_residual_noise(row):
            res = residuals_by_subsys.get(row["subsystem"])
            if res is None or len(res) == 0:
                return float(row["predicted_avg_mw"])
            sample = float(np.random.choice(res))
            return max(float(row["predicted_avg_mw"]) + args.noise_std * sample, 0.0)
        output_df["predicted_avg_mw"] = output_df.apply(add_residual_noise, axis=1)
