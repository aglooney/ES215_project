...
agg_fields = {"gen_val(MW)": "sum"}
for feat in WEATHER_FEATURES:
    agg_fields[feat] = "mean"

monthly = (
    raw.groupby(["subsystem", "year", "month"])
    .agg(agg_fields)
    .reset_index()
)
monthly["hours"] = monthly.apply(lambda r: month_hours(r["year"], r["month"]), axis=1)
monthly["avg_mw"] = monthly["gen_val(MW)"] / monthly["hours"]
...
for subsys, sub_df in monthly.groupby("subsystem"):
    ...
    if preds:
        all_rows.append(pd.DataFrame(preds))
